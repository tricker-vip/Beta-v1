...
logger("┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓", "[ KYPHAN ]");
  for(let i = 0; i <= admin.length -1; i++){
    dem = i + 1
    logger(` ID ADMIN ${dem}: ${(!admin[i]) ? "Trống" : admin[i]}`, "[ KYPHAN ]");
  }
  logger(` ID BOT: ${api.getCurrentUserID()}`, "[ KYPHAN ]");
  logger(` PREFIX: ${global.config.PREFIX}`, "[ KYPHAN ]");
  logger(` NAME BOT: ${(!global.config.BOTNAME) ? "GOJO KYPHAN" : global.config.BOTNAME}`, "[ KYPHAN ]");
  logger("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛", "[ KYPHAN ]");
  /////////////////////////////////////////////
  ...
  if ((event.body || '').startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.NDH.includes(event.senderID) && !global.config.ADMINBOT.includes(event.senderID)) {
   ...
   try {
     ...
     if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'callad') != event.args[0]) {
        if (!find_thuebot) return api.shareContact(`💸Thuê bot đi r dùng\n📝Chỉ 20k-1th\n🙅liên hệ admin để thuê`, global.config.NDH[0], event.threadID);
        if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now()) return api.shareContact(`💸Thuê bot đi r dùng\n📝Chỉ 20k-1th\n🙅liên hệ admin để thuê`, global.config.NDH[0], event.threadID);
     };
  };
