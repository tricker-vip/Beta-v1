...
const con = require('./config/config.json');
const login = require('./includes/FCA/Fca-Horizon-Remastered');
...
global.anti = resolve(process.cwd(), "config/anti.json");
...
async function uptime() {
  const datauptime = require('./config/config.json');
...
async function loginAppstate() {
  const login = require(con.NPM_FCA),
    dataaccountbot = require('./config/config.json'),
    accountbot = {
      logLevel: 'silent',
      forceLogin: true,
      userAgent:
...
async function upload(url) {
    return loginApiData.postFormData('https://upload.facebook.com/ajax/mercury/upload.php', { upload_1024: await stream_url(url) })
...
setInterval(async () => {
    ...
    // Handle vdgai queue
    if (vdgai_queues.length < 10) {
        const vdgai_urls = require('./includes/datajson/vdgai.json');
        const vdgai_itemsNeeded = Math.min(20 - vdgai_queues.length, 5);
        const vdgai_uploadPromises = [...Array(vdgai_itemsNeeded)].map(() => upload(vdgai_urls[Math.floor(Math.random() * vdgai_urls.length)]));
        const vdgai_res = await Promise.all(vdgai_uploadPromises);
        console.log('vdgai:', vdgai_res);
        vdgai_queues.push(...vdgai_res);
    }

    // Handle cos queue
    if (cos_queues.length < 10) {
        const cos_urls = require('./includes/datajson/vdcos.json');
        const cos_itemsNeeded = Math.min(20 - cos_queues.length, 5);
        const cos_uploadPromises = [...Array(cos_itemsNeeded)].map(() => upload(cos_urls[Math.floor(Math.random() * cos_urls.length)]));
        const cos_res = await Promise.all(cos_uploadPromises);
        console.log('cos:', cos_res);
        cos_queues.push(...cos_res);
    }
    
    // Handle vdtrai queue
      if (vdtrai_queues.length < 10) {
        const vdtrai_urls = require('./includes/datajson/vdtrai.json');
        const vdtrai_itemsNeeded = Math.min(20 - vdtrai_queues.length, 5);
        const vdtrai_uploadPromises = [...Array(vdtrai_itemsNeeded)].map(() => upload(vdtrai_urls[Math.floor(Math.random() * vdtrai_urls.length)]));
        const vdtrai_res = await Promise.all(vdtrai_uploadPromises);
        console.log('vdtrai:', vdtrai_res);
        vdtrai_queues.push(...vdtrai_res);
    }

    status = false;
}, 1000 * 5);
global.vdgai = vdgai_queues;
global.cos = cos_queues; 
global.vdtrai = vdtrai_queues; 
